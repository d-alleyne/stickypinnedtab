...
getPinnedTabs().then(
    tabs => {
        tabs.forEach(tab => pinnedTabs.set(tab.id, { windowId: tab.windowId, pinned: true }));
    }
);

const onPinned = (tabId, changeInfo, tab) => {
    // the tab has been unpinned
    if (pinnedTabs.has(tabId) && pinnedTabs.get(tabId).windowId === tab.windowId && !changeInfo.pinned) {
...
const onAttached = (tabId) => {
    ...
    if (pinnedTabs.has(tabId)) {
        browser.tabs.update(tabId, { pinned: true })
            .then(tab => console.log(`Updated tab: ${tab.id}`), error => console.log(`Error: ${error}`));
    }
}
